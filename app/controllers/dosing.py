...
@dosing_bp.route('/profiles/migrate', methods=['POST'])
def migrate_profiles():
    """Migrate plant profiles to use the new nutrient pump types"""
    # Get all plant profiles
    plant_profiles = Settings.get('plant_profiles', {})
    
    # Counter for migrated profiles
    migrated_count = 0
    
    # Get all current nutrient pumps
    nutrient_pumps = {pump.id: pump for pump in Pump.query.filter(Pump.type == 'nutrient').all()}
    
    # For each profile
    for profile_id, profile in plant_profiles.items():
        updated = False
        
        # Check if it has nutrient components
        if 'nutrient_components' in profile and profile['nutrient_components']:
            new_components = []
            
            for component in profile['nutrient_components']:
                pump_id = component.get('pump_id')
                
                # Check if the pump exists
                pump = Pump.query.get(pump_id)
                
                if pump:
                    # If it's a new nutrient pump, keep it
                    if pump.type == 'nutrient':
                        new_components.append(component)
                    else:
                        # Old pump type - see if we can find a replacement
                        if nutrient_pumps:
                            # Use the first available nutrient pump
                            new_pump_id = next(iter(nutrient_pumps.keys()))
                            new_pump = nutrient_pumps[new_pump_id]
                            
                            # Create a new component with the new pump
                            new_component = {
                                'pump_id': new_pump_id,
                                'pump_name': new_pump.name,
                                'ratio': component.get('ratio', 1.0)
                            }
                            new_components.append(new_component)
                            updated = True
                else:
                    # Pump doesn't exist anymore - see if we can find a replacement
                    if nutrient_pumps:
                        # Use the first available nutrient pump
                        new_pump_id = next(iter(nutrient_pumps.keys()))
                        new_pump = nutrient_pumps[new_pump_id]
            ...
            # If no valid components were found and we have nutrient pumps, add one
            if not new_components and nutrient_pumps:
                new_pump_id = next(iter(nutrient_pumps.keys()))
                new_pump = nutrient_pumps[new_pump_id]
            ...
            # Update the profile with the new components
            if updated:
    ...
    if migrated_count > 0:
        Settings.set('plant_profiles', plant_profiles)
        flash(f'Successfully migrated {migrated_count} plant profiles to use the new nutrient pump types', 'success')
    else:
        flash('No plant profiles needed to be migrated', 'info')
